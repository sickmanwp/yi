...
注:正则表达式注释语法`(?#注释内容)`中，并未要求注释内容与`#`间要有一个空格，但为了美观`SCP-079-REGEX`会在此自动加上一个空格。
